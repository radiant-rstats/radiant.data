...
#'
#' @param dataset Data set to use for plot (data.frame or tibble) 
  ...
  if (type == "dist") {
    for (i in xvar) {
      ...
      if ("factor" %in% dc[i]) {
        ...
        hist_par[["binwidth"]] <- select_at(dat, .vars = i) %>% range() %>% {
          diff(.) / (bins - 1)
        }
      }
    ...
    for (i in xvar) {
      ...
      for (j in yvar) {
        ...
        if (dc[i] == "factor") {
          ...
          ymax <- max(0, max(dat[[j]]))
          ymin <- min(0, min(dat[[j]]))
