...
stop_radiant <- function() {
  ...
  if (interactive()) {
    if (isTRUE(getOption("radiant.launch", "browser") == "browser")) {
    ...
    } else if (isTRUE(getOption("radiant.launch", "browser") == "window")) {
      showModal(
        modalDialog(
          title = "Close Radiant in Rstudio Window",
          span(
            "Currently, the best way to close Radiant when running in an 
             Rstudio Window is to close the Window or to click the (red) 
      ...
      return(invisible())
    } else if (isTRUE(getOption("radiant.launch", "browser") == "viewer")) {
      showModal(
        modalDialog(
          title = "Close Radiant in Rstudio Viewer",
          span(
            "Currently, the best way to close Radiant when running in the 
             Rstudio Viewer is to click the red stop button in Rstudio"
          ),
          footer = modalButton("OK"),
          size = "s",
          easyClose = TRUE
